...
from utils import load_pathway, get_match_id
...
parser.add_argument('--gene_data', type=str, required=True)
parser.add_argument('--other_data', type=str, required=True)
parser.add_argument('--pathway_data', type=str, required=True)
parser.add_argument('--pmvae_dim', type=int, required=True)
parser.add_argument('--pmvae_path', type=str, required=True)

args = parser.parse_args()

print(f"Received: {args.index}, {args.gene_data}, {args.other_data}, {args.pathway_data}, {args.pmvae_dim}, {args.pmvae_path}")

loaded_indices = np.load(args.index)
train_idx = loaded_indices['train_idx']
eval_idx = loaded_indices['eval_idx']


# load_data_deepshap: transfer gene and olink data into tensor
def load_data_deepshap(data1, data2, dtype):
    EXPR1 = data1.iloc[:,1:].values.astype(np.float64)
    EXPR2 = data2.iloc[:,1:].values.astype(np.float64)
    EXPR1 = torch.from_numpy(EXPR1).type(dtype)
    EXPR2 = torch.from_numpy(EXPR2).type(dtype)

    return (EXPR1, EXPR2)
train_data_gene_pre = pd.read_csv(args.gene_data)
train_data_mirna_pre = pd.read_csv(args.other_data)
train_data_gene = train_data_gene_pre.iloc[train_idx,:].reset_index(drop=True)
train_data_mirna = train_data_mirna_pre.iloc[train_idx,:].reset_index(drop=True)
gene_names = list(train_data_gene.columns)[1:]
mirna_names = list(train_data_mirna.columns)[1:]
feature_names = gene_names + mirna_names

# In[ ]:
upper_data = pd.read_csv("data/higher_PI_train.csv")
lower_data = pd.read_csv("data/lower_PI_train.csv")

# In[ ]:


pathway_mask = pd.read_csv(args.pathway_data, index_col = 0)
pathway_names = list(pathway_mask.columns)
mask_names = pathway_names + mirna_names
pathway_mask_int = load_pathway(args.pathway_data, dtype)

"""
"""
# number of genes
input_n1 = len(gene_names)
# number of miRNAs
input_n2 = len(mirna_names)
# number of latent features: one of the hyperparameters, determined via grid search during training/validation process
z_dim = args.pmvae_dim
...
def splitExprandSample_new(condition_data, sampleSize, expr):
...
def UnSupShapExplainer_CPU_Only(train_overall_df_gene, train_overall_df_mirna, condition_data1, condition_data2, 
                                path, file_count, dimension, featureNames, maskNames, imp_feature, imp_mask):

    # Load model and force it to CPU
    KL_PMVAE_model = KL_PMVAE_2omics(z_dim, input_n1, input_n2, pathway_mask_int)
    KL_PMVAE_model.load_state_dict(torch.load(args.pmvae_path, map_location=torch.device('cpu')))
    KL_PMVAE_model = KL_PMVAE_model.cpu()  # Explicitly move to CPU

    # Ensure pathway_mask is also on CPU if it's a tensor
    if hasattr(KL_PMVAE_model, 'pathway_mask') and torch.is_tensor(KL_PMVAE_model.pathway_mask):
        KL_PMVAE_model.pathway_mask = KL_PMVAE_model.pathway_mask.cpu()

    print(f"Model is on device: {next(KL_PMVAE_model.parameters()).device}")

    # Rest of the function remains the same...
    samplesize_gene = condition_data1.shape[0]//2-5
    upper_group_sample_gene = splitExprandSample_new(condition_data=condition_data1, sampleSize=samplesize_gene, expr=train_overall_df_gene)
    ...
    samplesize_mirna = condition_data2.shape[0]//2-5    
    lower_group_sample_gene = splitExprandSample_new(condition_data=condition_data2, sampleSize=samplesize_mirna, expr=train_overall_df_gene)
    ...
    if imp_feature:
        e = PyTorchDeepExplainer(KL_PMVAE_model, lower_group_sample_gene, lower_group_sample_mirna, 
                                outputNumber=0, dim=dimension, explainLatentSpace=True)
    if imp_mask:
        e = PyTorchDeepExplainer((KL_PMVAE_model, KL_PMVAE_model.e_fc2_mean), lower_group_sample_gene, lower_group_sample_mirna, 
                                outputNumber=0, dim=dimension, explainLatentSpace=True)
    ...
    most_imp = getTopShapValues(shap_vals=shap_values_obtained, numberOfTopFeatures=10, path=path, 
                               file_count=file_count, featureNames=featureNames, maskNames=maskNames, 
                               imp_feature=imp_feature, imp_mask=imp_mask, absolute=True)
...
def getTopShapValues(shap_vals, numberOfTopFeatures, path, file_count, featureNames, maskNames = None, imp_feature = True, imp_mask = False, absolute=True):
    ...
    feature_importance.to_csv(path + "/PMVAE_imp_" + file_count + "_without_musk.csv")
...
# Filter features that start with 'Z_' and get top 20
lfbin_shap = pd.read_csv('deepshap/lfbin_imp.csv')
z_features = [feature for feature in lfbin_shap['features'] if feature.startswith('Z_')]
important_latent = z_features[:20]

imp_features_cache = []
for i in important_latent:
    print("Latent dimension: " + i + ".")
    temp_dim = int(i.split("_")[1])-1
    temp_most_imp_fea = UnSupShapExplainer_CPU_Only(train_overall_df_gene=train_data_gene, train_overall_df_mirna=train_data_mirna, condition_data1=upper_data, condition_data2=lower_data, 
                                           path='deepshap', file_count = i,
...
big_freq_summary.to_csv("deepshap/1_pmvae__import_biomarker_frequency_without_mask.csv")
...
def getTopShapValues(shap_vals, numberOfTopFeatures, path, file_count, featureNames, maskNames = None, imp_feature = True, imp_mask = False, absolute=True):
    ...
    feature_importance.to_csv(path + "/PMVAE_imp_" + file_count + "_with_musk.csv")
...
for i in important_latent:
    ...
    temp_most_imp_fea = UnSupShapExplainer_CPU_Only(train_overall_df_gene=train_data_gene, train_overall_df_mirna=train_data_mirna, condition_data1=upper_data, condition_data2=lower_data, 
                                           path='deepshap', file_count = i,
                                           dimension=temp_dim, featureNames=feature_names, maskNames=mask_names, imp_feature=True, imp_mask=True)
    imp_features_cache = imp_features_cache + list(temp_most_imp_fea.features)
...
big_freq_summary.to_csv("deepshap/1_pmvae__import_biomarker_frequency_with_mask.csv")